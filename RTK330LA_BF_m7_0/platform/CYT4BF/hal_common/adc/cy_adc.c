...
*       idle or a econtinuousf triggered group completes.
...
{
    cy_en_adc_status_t    ret      = CY_ADC_SUCCESS;
    un_PASS_SAR_DIG_CAL_t unDigCal = base->unDIG_CAL;
    if (NULL != config)
    {
...
{
    cy_en_adc_status_t        ret      = CY_ADC_SUCCESS;
    un_PASS_SAR_DIG_CAL_ALT_t unDigCal = base->unDIG_CAL_ALT;
...
{
    cy_en_adc_status_t    ret      = CY_ADC_SUCCESS;
    un_PASS_SAR_ANA_CAL_t unAnaCal = base->unANA_CAL;
...
{
    cy_en_adc_status_t        ret      = CY_ADC_SUCCESS;
    un_PASS_SAR_ANA_CAL_ALT_t unAnaCal = base->unANA_CAL_ALT;
...
double Cy_Adc_CalculateDieTemperature(cy_stc_adc_temp_ref_t *refValue, cy_stc_adc_temp_raw_t *rawValue)
{
    ...
    double coefficientMatrixForClass[3][3] = {{0.0},{0.0},{0.0}};
    
    double sortTempMatrixA[3][3];
    double sortTempValueMatrixB[3][1];
    double sortTempCoefficeintsX[3] = {0.0};
    
    double classTempMatrixA[2][2];
    double classTempValueMatrixB[2][1];
    double classTempCoefficeintsX[2] = {0.0};
    ...
    sortTempMatrixA[0][1] = refValue->adcTempSortValues.coldTempValue;
    sortTempMatrixA[0][2] = pow(refValue->adcTempSortValues.coldTempValue, 2.0);
    sortTempMatrixA[1][0] = 1.0;
    sortTempMatrixA[1][1] = refValue->adcTempSortValues.roomTempValue;
    sortTempMatrixA[1][2] = pow(refValue->adcTempSortValues.roomTempValue, 2.0);
    sortTempMatrixA[2][0] = 1.0;
    sortTempMatrixA[2][1] = refValue->adcTempSortValues.hotTempValue;
    sortTempMatrixA[2][2] = pow(refValue->adcTempSortValues.hotTempValue, 2.0);
    
    /* Update the sort temperature adc value matrix B */
    sortTempValueMatrixB[0][0] = refValue->adcTempSortValues.coldAdcValue;
    sortTempValueMatrixB[1][0] = refValue->adcTempSortValues.roomAdcValue;
    sortTempValueMatrixB[2][0] = refValue->adcTempSortValues.hotAdcValue;
    
    /* Update the class temperature value matrix A */
    classTempMatrixA[0][0] = 1.0;
    classTempMatrixA[0][1] = refValue->adcTempClassValues.roomTempValue;
    classTempMatrixA[1][0] = 1.0;
    classTempMatrixA[1][1] = refValue->adcTempClassValues.hotTempValue;
    
    /* Update the class temperature adc value matrix b */
    classTempValueMatrixB[0][0] = refValue->adcTempClassValues.roomAdcValue;
    classTempValueMatrixB[1][0] = refValue->adcTempClassValues.hotAdcValue;
    ...
    /*************************************************************************** 
     * Get the 2nd order coefficient for class value matrix
     **************************************************************************/
    
    /* Add a correction to class temperature adc value matrix B */
    classTempValueMatrixB[0][0] -= (sortTempCoefficeintsX[2])*(pow(classTempMatrixA[0][1], 2.0));
    classTempValueMatrixB[1][0] -= (sortTempCoefficeintsX[2])*(pow(classTempMatrixA[1][1], 2.0));
    
    /* Get the determinant of class temperature adc value matrix A */
    determinant = classTempMatrixA[0][0]*classTempMatrixA[1][1] - classTempMatrixA[1][0]*classTempMatrixA[0][1];
  
    /* Get the inverse of class temperature adc value matrix A */
    coefficientMatrixForClass[0][0] = classTempMatrixA[1][1]/determinant;
    coefficientMatrixForClass[0][1] = -classTempMatrixA[0][1]/determinant;
    coefficientMatrixForClass[1][0] = -classTempMatrixA[1][0]/determinant;
    coefficientMatrixForClass[1][1] = classTempMatrixA[0][0]/determinant;
    for(uint8_t i = 0u; i < 2u; i++)
    {
        for(uint8_t j = 0u; j < 2u; j++)
        {
            classTempMatrixA[i][j] = coefficientMatrixForClass[i][j];
        }
    }
    
    /* Calculate class temperature adc vale coefficient matrix X = (invA)*B */
    for(uint8_t i = 0u; i < 2u; i++)
    {
        for(uint8_t j = 0u; j < 2u; j++)
        {
            classTempCoefficeintsX[i] += (classTempValueMatrixB[j][0]*classTempMatrixA[i][j]);
        }
    }
    ...
    correctionFactor = (refValue->adcVbgValue) / (rawValue->adcVbgRawValue);
    ...
    coeffB = classTempCoefficeintsX[1];
    coeffC = classTempCoefficeintsX[0];
    ...
    tempRootPos = ((-coeffB)+(sqrtf(bSqrMin4ac))) / ((2.0)*(coeffA));     // this can be minimize, kept for comparition
    
    /* Select the root that lies between the Hot and Cold temperature sort values [-40 degC, 150 degC] */
    if((tempRootPos < (refValue->adcTempSortValues.hotTempValue)) && (tempRootPos > (refValue->adcTempSortValues.coldTempValue)))
    {
        /* Temperature value is positive root of the curve */
        tempInDegreeC = tempRootPos;
    }
    else if((tempRootNeg < (refValue->adcTempSortValues.hotTempValue)) && (tempRootNeg > (refValue->adcTempSortValues.coldTempValue)))
    {
