...
VERSION_RTK_MINOR      :=  0
VERSION_RTK_PATCH      :=  0
VERSION_RTK   :=  ${VERSION_RTK_MAJOR}.${VERSION_RTK_MINOR}.${VERSION_RTK_PATCH}

# C thumb sources
BUILD_CSOURCES:=$(foreach src,${PLATFORM_C_SOURCES} ${APP_C_SOURCES} ${GNSS_C_SOURCES} ,${INS401_RTK_APP_ROOTDIR}/$(src))
BUILD_CPPSOURCES:=$(foreach src,${PROJ_CPPSOURCES},${INS401_RTK_APP_ROOTDIR}/$(src))

# C ARM sources
BUILD_CARMSOURCES:=$(foreach src,${CYT4BFBCHD_M7_1_ASMSOURCES},${INS401_RTK_APP_ROOTDIR}/$(src))

# Assembly sources
BUILD_ASMSOURCES:=$(foreach asm,${CYT4BFBCHD_M7_1_ASMSOURCES},${INS401_RTK_APP_ROOTDIR}/platform/$(asm))
...
ifneq (${VERSION_RTK},)
VERSION_CDEFS+=VERSION_RTK_APP=${VERSION_RTK}
VERSION_CDEFS+=TIME_RTK_APP=${COMPILE_TIME}
VERSION_CDEFS+=AUTHOR_RTK_APP=${COMPILER_AUTHOR}
CDEFS+= $(foreach def,$(strip ${VERSION_CDEFS} ),-D${def})
${info rtk app version = ${VERSION_RTK}}
endif
...
APP_MAPS:= ${TARGET_ROOTDIR}/rtk.map
LINK_FILE:= ${INS401_RTK_APP_ROOTDIR}/../linker/linker_directives_tviibh.icf
#		${ASMLIBS}
${TARGET}:  ${PROJ_COBJS}  ${PROJ_CPPOBJS} ${PROJ_ASMOBJS}
	$(call MK_ECHO,Linking to $(subst ${INS401_RTK_APP_ROOTDIR}/,,$@))
...
ifeq "$(APPEND_COMPILE_MESSAGE)" "TRUE"
	...
	$(call MK_APPEND,verion: rtk_app_${VERSION_RTK},"${APP_MESSAGE_FILE}")
