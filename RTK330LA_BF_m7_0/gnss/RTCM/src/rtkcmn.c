...
RTK_RAM_CODE int satno(int sys, int prn)
{
    ...
    case _SYS_BDS_:
        ...
        // 		return 0;
        // 	return NSATGPS + NSATGLO + NSATGAL + NSATBDS + prn - MINPRNLEO + 1;
        // case _SYS_SBS_:
        // 	if (prn < MINPRNSBS || MAXPRNSBS < prn)
        // 		return 0;
        // 	return NSATGPS + NSATGLO + NSATGAL + NSATBDS + NSATLEO + prn - MINPRNSBS + 1;
    default:
        return 0;
    }
    // return 0;	
...
* return : gtime_t struct
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern gtime_t gst2time(int week, double sec)
{
...
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern double time2gst(gtime_t t, int* week)
{
...
/* compare satellite through number */
extern int cmpsat(const void* p1, const void* p2)
{
    obsd_t* q1 = (obsd_t*)p1, * q2 = (obsd_t*)p2;
    int ret;

    ret = (int)(q1->sat - q2->sat);
...
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern void xyz2enu_(const double* pos, double* E)
{
...
* return : none
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern void ecef2enu(const double* pos, const double* r, double* e)
{
    double E[9];
...
* notes  : matirix stored by column-major order (fortran convention)
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern void xyz2enu(const double* pos, double* E)
{
    double sinp = sin(pos[0]), cosp = cos(pos[0]), sinl = sin(pos[1]), cosl = cos(pos[1]);
...
* return : none
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern void enu2ecef(const double* pos, const double* e, double* r)
{
    double E[9];
...
* return : none
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern void covecef(const double* pos, const double* Q, double* P)
{
    double E[9], EQ[9];
